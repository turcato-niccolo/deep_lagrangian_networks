...
from deep_lagrangian_networks.utils import load_dataset_panda, init_env
...
if __name__ == "__main__":
    ...
    # Read the dataset:
    n_dof = 7
    train_data, test_data = load_dataset_panda()
    train_qp, train_qv, train_qa, train_tau = train_data
    test_qp, test_qv, test_qa, test_tau = test_data
    ...
    if load_model:
        load_file = "data/FE_panda_delan_model.torch"
    ...
    while epoch_i < hyper['max_epoch'] and not load_model:
        ...
        if epoch_i == 1 or np.mod(epoch_i, 1) == 0:
    ...
    if save_model:
        torch.save({"epoch": epoch_i,
                    ...
                    "data/FE_panda_delan_model.torch")
    ...
    plt.figure('Torque performance')
    for i in range(n_dof):
        plt.subplot(4,2,i+1)
        plt.plot(train_tau[:,i], 'r', label='True')
        plt.plot(delan_tau[:,i], 'b', label='DeLaN')
    plt.subplot(4,2,7)
